...
// 名片3D坐标
var objects = [];
var targets = { sphere: [], grid: [], table: [] };

// 动画类型
var animateTypes = ["sphere", "grid", "none", "table"];
...
function setAnimate(type) {
  ...
  if (type === "sphere") {
    ...
    // 有序的悬浮
    transform(targets.table, animateDuration);

    new TWEEN.Tween(ballGroup.rotation)
      .to(
        {
          x: 0,
          y: Math.round(ballGroup.rotation.y / (Math.PI * 2)) * Math.PI,
          z: 0,
        },
        animateDuration
      )
